...
```
     📥 全材料导入枢纽                    🔍 结构化解构与清洗                 📖 双语沉浸研学
 [系列网页 / 知识库文件夹 / EPUB] ──▶  [目录树嗅探 / 保序并发抓取]  ──▶  [句段平行中英排版]
 [PDF / Word / MD / TXT]              [跨页断行缝合 / 图片离线缓存]            │
                                                                           ▼
     🌐 随身离线成果大厅                  🗂️ 科学内化与巩固                 🤖 即时伴读深度追问
  [单文件/多文档纯静态SPA]   ◀──  [SM-2 遗忘曲线自测闪卡]  ◀──  [就地费曼式答疑解惑]
```

1. **结构拆解与全材料兼容**：精准提取 PDF、Word、EPUB、Markdown 资料包以及系列网页技术文档的层级章节、代码块与公式图表；
2. **保序并发与高可用自愈**：8 路异步受控并发抓取大型 Web 文档站（如 1000+ 章节），内置 25 秒超时隔离、自动重试、在线图片 Fallback 与单章增量补抓；
3. **双语精读**：段落级中英平行排版，默认墨玉羊皮等 4 套经典研学护眼主题，配合向下滚动自动隐藏顶栏，沉浸专注；
4. **即时伴读追问（核心）**：段落级带上下文滑动窗口的专属研学私教，随时对长难句、核心论点、工程权衡发起就地费曼式追问与概念解构；
5. **科学自测内化**：从核心概念中提炼互动 3D 闪卡与镂空挖空题（Cloze），基于 **SuperMemo SM-2** 间隔重复算法进行主动回忆自测，巩固长期记忆；
6. **离线成果携带**：所有双语对照、伴读解析与自测卡片均可一键导出为零外部依赖的单文件或离线静态研学成果大厅，断网随身可用，直传 Cloudflare Pages 或 GitHub Pages。
...
* **📥 全材料导入枢纽 (Omni-Material Import Hub)**：
  点击左侧 **【导入研学材料】**，支持统一导入多达 4 类知识载体：
  * **🌐 知识性系列网页 / 文档站 (Web Series)**：
    * 智能嗅探文档框架侧边栏（MkDocs、Docusaurus、GitBook、Sphinx 等）；
    * 自动圈选所属专题模块姊妹篇（如 OWASP MASTG 专题），支持在树状目录中自由多选或全选；
    * **8 路保序受控并发**：1000+ 章节大文档抓取时间由 1+ 小时缩短至 5~8 分钟，严格保持原目录序号；
    * **单页 25 秒熔断与抖动重试**：网络偶发抖动自动重试，持续超时单页熔断，彻底告别整书挂起卡死；
    * **图片双轨 Fallback**：图片下载失败自动保留源站公网绝对 URL，在线直连展示，绝不出现 404 裂图；
    * **失败按需重抓**：侧边栏自动检测未完成或超时的章节，显示【重抓失败 (X章)】，极速增量补齐。
  * **📁 Markdown 资料文件夹 / 压缩包 (Folder & Zip)**：
    * 支持在浏览器中**直接选择本地项目文件夹**或上传 `.zip` 压缩包；
    * 自动识别 `SUMMARY.md`、`mkdocs.yml`、`_sidebar.md` 或数字前缀文件树；
    * 自动识别并迁移包内 `images/` 等静态图片附件。
  * **📚 EPUB 原生电子书 (E-Book)**：
    * 纯原生无损解包容器，提取书名、作者与 NCX/NAV 目录树，将各章节与插图按阅读流转化为标准教材。
  * **📄 常规学术文档**：
    * 原生支持 **PDF（文字版/扫描版）、Word (DOCX)、单 Markdown、TXT**。
* **⚡ 批量翻译与双语平行排版**：
  * 点击顶栏 **【⚡ 批量翻译】** 或章节头部的 **【⚡ 批量翻译本章】**，系统按当前绑定的翻译模型逐段并行沉浸式翻译；
  * 原文与译文采用段落级平行排版，默认**墨玉羊皮（Sepia）**、**深邃夜色（Dark）**等 4 套护眼主题，版心宽度自适应调节；
  * 阅读时向上滚动自动显现导航顶栏，向下深入阅读时自动渐隐，提供极致沉浸感。
* **🤖 段落 Chat（AI 伴读助教）**：
...
### 2. 全材料学术与技术工程解析引擎 (Import Hub)
- **多形态全材料支持**：
  - **技术文档站 (Web Series)**：原生支持 MkDocs、Docusaurus、GitBook、Sphinx 目录嗅探与层级多选；
  - **资料包 (Folder & Zip)**：一键拖入 Markdown 文件夹或压缩包，智能识别 `SUMMARY.md`/`mkdocs.yml`；
  - **电子书 (EPUB)**：纯原生无损解析书籍元数据、NCX 目录与插图；
  - **学术文献**：高保真解析 PDF（文字版/扫描版）、Word (DOCX) 及 TXT。
- **高保真图表与双轨 Fallback**：本地化离线嵌入插图，若源站下载超时自动降级保留公网绝对 URL 在线直连，绝不裂图；
- **排版断裂自动修复**：自动识别 PDF 换行误切断词并无缝缝合，还原流畅语义流；
- **8路保序并发与故障自愈**：上千章节大型文档受控并发高速抓取，单页 25 秒超时熔断防死锁，支持侧边栏【失败重抓】一键按需补齐。
